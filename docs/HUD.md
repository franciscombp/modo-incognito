...
| EL PULGAR MANDA EN TODO | ✅ | La palanca navega menús (`focusNav.empujar`), el botón acepta Y pasa diálogos (`dialogue.avanzar`), y en el BAILE la palanca ES un paso (empujón en flanco). Una pantalla que se juega con direcciones se marca `data-nav-juego` y el cursor la deja en paz — sin eso, el botón CERRABA el minijuego (su única opción navegable era SALIR). Y en cualquier minijuego la zona del stick se ENCOGE: medido en un Pixel 7, tapaba las casillas del baile, los vasos y el chisme. Lo vigilan `check:pulgar` y `check:baile-pulgar` |
| LA CÁMARA DE CONVERSACIÓN, en dos planos | ✅ | `setCinematic` es GRADUADO: 1 = soliloquio (primer plano, cuarta pared), 0.62 = dúo (plano medio con el foco corrido hacia la cámara — `DUO_ALZA` — para que la pareja quede SOBRE la caja de texto; quien está del lado del espectador caía justo detrás de ella). El interlocutor se resuelve POR NOMBRE del speaker si la escena no declaró reparto, con guarda de distancia; y `setHeading` acepta `snap` porque el diálogo corre en pausa y el tween del giro no avanza |
| EL ANUNCIO se coloca MIDIENDO | ✅ | `.inc-msg-centro` mide la lista de misiones (que no tiene alto fijo) y se pone debajo, con su propio alto de maqueta (`offsetHeight` — el rect llega escalado por la animación) y el pico del rebote. Un porcentaje fijo volvió a solaparse en cuanto la lista tuvo una fila más. Lo vigila `check:encimados` |
...
| El diálogo estilo Sasquatch | ✅ | `dialogue.js` + bloque `.inc-dialogue-*`: caja sólida oscura abajo al centro, NOMBRE en píldora sobre el filo, texto grande centrado, triángulo asomando abajo. El retrato 3D pasó de telón central (48vw×84vh) a busto moderado a la izquierda; las réplicas del jugador firman con el NOMBRE del personaje elegido (`ctx.getPlayerName`), no «Tú». Los diálogos van EN COLA (dos `play()` a la vez se pisaban la caja y quedaba colgada — pasaba cada vez que un secuaz abordaba en el mismo instante del regaño) y la máquina de escribir va POR TIEMPO, no por tick (con timers estrangulados una línea tardaba medio minuto) |
| La CARA de la placa, dentro de su marco | ✅ **arreglado** | `.inc-plate-face` no tenía `position: relative`, y el lienzo del retrato (`.vn-portrait-canvas`, que es `absolute; inset:0` para llenar su marco en el diálogo) se anclaba al ancestro posicionado de más arriba: se estiraba a la PLACA ENTERA y tapaba rombos y energía. `overflow: hidden` no lo recortaba porque un absoluto anclado más arriba ni siquiera cuenta como contenido de esa caja |
| El rastreador de TAREA, retirado de la rutina | ✅ | Desde que la lista lleva aguja y metros por fila, esa flecha del borde repetía lo mismo: la misma distancia salía tres veces y las dos flechas del borde acababan apiladas en una esquina. Ahora el borde se reserva para lo que la lista NO dice — la HUIDA (sospecha alta → lugar seguro) y el JEFE. Lo planificado se lee en la lista; el borde solo grita lo urgente |
| LAS CARITAS del escaqueo | ✅ | El reverso exacto del Zzz (`createHappyIcon` en `alertIcon.js`): tres caritas con los ojos cerrados de gusto que suben mientras ACTIVAS una actividad o la AGUANTAS encendida. Los dos globos hablan de TU estado y comparten gramática —suben, crecen, se desvanecen—, así que juntos cuentan el bucle sin explicar ninguno: el Zzz dice «el día me pudo», las caritas «me lo estoy pasando bien». Fingir NO las saca: es lo que más cansa, y premiarlo contradiría el chiste. Se dibujan a mano porque la carita de Phosphor ya es el icono del JEFE |
| La AGUJA de cada misión | ✅ | `.inc-quest-aguja`: hacia DÓNDE queda, no solo cuán lejos — un «17 m» sin rumbo obliga a dar vueltas para descubrir por dónde. Usa `groundToScreen`, la MISMA proyección que los mandos, así que apunta a donde de verdad hay que empujar el stick; se apaga a menos de 1,5 m (ahí manda la medalla). El icono de Phosphor apunta al NOROESTE en su forma original, de ahí el `+45°` — un desfase de flecha se ve «casi bien» y manda a la pared de al lado, así que lo vigila `npm run check:rumbo` contra el banco de pruebas |
| La BARRA de progreso de la fila | ✅ **arreglada** | Era lo que «no se entendía», y con razón: **mentía**. `o.progress` va de 0 a `o.time` en SEGUNDOS, pero se pintaba como si fuera fracción 0–1 (`progress * 100`), así que en una tarea de 6 s marcaba el tope al primer segundo y vivía llena bajo cada título sin informar de nada. Ahora es la fracción real, más gruesa (6 px), y **solo sale con la tarea a medias** |
| LA GUÍA de la misión seguida | ✅ | `.inc-quest-guide` bajo el título de la fila seguida: el SIGUIENTE PASO exacto, calculado por frame en `game._guiaSeguida()` («consigue el café hablándole al Parce», «mantén espacio», «aguanta», y en caza «corre a un lugar seguro»). Es la mano que lleva; el repliegue por tensión también la esconde |
